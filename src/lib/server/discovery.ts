...
export const discoverServices = createServerFn({ method: "POST" }).handler(
  async () => {
    ...
    for (const integration of dockerIntegrations) {
      try {
        const containers = await getDockerContainers({
          data: { integrationId: integration.id, all: true },
        });
        if (Array.isArray(containers)) {
          for (const container of containers as DockerContainer[]) {
            allServices.push(
              dockerToDiscoveredService(container, integration.id, integration.name)
            );
          }
        }
      } catch (error) {
    ...
    for (const integration of truenasIntegrations) {
      try {
        const apps = await getTrueNASApps({
          data: { integrationId: integration.id },
        });
        if (Array.isArray(apps)) {
          for (const app of apps as TrueNASApp[]) {
            allServices.push(
              truenasToDiscoveredService(app, integration.id, integration.name)
            );
          }
        }
      } catch (error) {
